...
public class VoteLinkValidator implements Validator<VoteLinkDto> {
    ...
    public void validate(VoteLinkDto voteLinkDto) throws ValidationException {
        ...
        if (Stream.of(voteLinkDto.getAuctionItemId(), voteLinkDto.getVoteReceiverId()).anyMatch(Objects::isNull))
            throw new ValidationException(UserError.MISSING_DATA);
        if (Stream.of(voteLinkDto.getAuctionItemId(), voteLinkDto.getVoteReceiverId()).anyMatch(String::isEmpty))
