...
public class SessionRecommendation {
	...
	public void recommendItems(Map<String, Set<String>> preferredAuctionsPerUser,
			Map<String, Integer> bidsOrBuyoutPerAuction, LocalDateTime lastRun, String userId) {
	...
	private List<String> findRecommendedItems(String userId) {
		...
		kMostCommonUsers.forEach(commonUser -> {
			...
			List<String> recommendedAuctions = auctionsOfCommonUser.stream()
					.filter(auctionOfCommonUser -> !preferredAuctionsPerUser.get(userId).contains(auctionOfCommonUser))
		...
		if (kMostCommonAuctionItems.size() < recommendedItems) {
			...
			Set<String> auctionItemIds = activeAuctions.stream().map(AuctionItem::getAuctionItemId)
		...
		return kMostCommonAuctionItems.subList(0,
				recommendedItems >= kMostCommonAuctionItems.size() ? kMostCommonAuctionItems.size() : recommendedItems);
	}

	// private List<String> findRecommendedItems(String userId, List<String>
	// kMostCommonUsers) {
	// List<String> kMostCommonAuctionItems = new LinkedList<>();
	// List<AuctionItem> activeAuctions =
	// auctionItemRepository.findAuctionItemIdsOfActiveAuctions(new Date());
	// Set<String> auctionItemIds =
	// activeAuctions.stream().map(AuctionItem::getAuctionItemId).collect(Collectors.toSet());
	// List<DoubleList> similaritiesPerAuction = new LinkedList<>();
	// Set<String> auctionItemIdsOfUsers = new HashSet<>();
	// kMostCommonUsers.forEach(id ->
	// auctionItemIdsOfUsers.addAll(preferredAuctionsPerUser.get(id)));
	// for (String auctionItemId : auctionItemIds) {
	// similaritiesPerAuction.clear();
	// auctionItemIdsOfUsers.forEach(auctionItemIdOfUsers -> {
	// Double similarity = similarityOfAuctions(auctionItemId,
	// auctionItemIdOfUsers);
	// similaritiesPerAuction.add(new DoubleList(similarity,
	// auctionItemIdOfUsers));
	// });
	...
	// int pos = rng.nextInt(auctionItemIds.size());
	// if
	// (!kMostCommonAuctionItems.contains(activeAuctions.get(pos).getAuctionItemId()))
	// {
	// itemNo--;
	// kMostCommonAuctionItems.add(activeAuctions.get(pos).getAuctionItemId());
	// }
	...
	private List<String> findKMostCommonUsers(String userId) {
		...
		return similarities.subList(0, k >= similarities.size() ? similarities.size() : k).stream()
				.map(DoubleList::getId).collect(Collectors.toList());
	}
